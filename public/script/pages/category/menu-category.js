$(document).ready(function () {
    const table = $('#placeMenuCategoriesTable');

    table.DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "/_route/datatables/elasticsearch/menu_category",
            type: "POST",
            dataType: "json",
            dataSrc: "data",
    ...
    table.on('click', '.delete-btn', function () {
        const id = $(this).data('id');
        if (confirm("Bu menü kategorisini silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.")) {
            $.ajax({
                url: `/_route/api/api/category/menus/${id}`, type: 'DELETE', success: function (result) {
                    console.info(result);
    ...
    $('#editForm').on('submit', function (e) {
        ...
        $.ajax({
            url: `/_route/api/api/category/menus/${id}`,
            type: 'PATCH',
    ...
    $('#addForm').on('submit', function (e) {
        ...
        $.ajax({
            url: '/_route/api/api/category/menus',
            type: 'POST',
