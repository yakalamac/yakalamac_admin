$(document).ready(function () {
    ...
    table.DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "/_route/datatables/elasticsearch/item_category",
            type: "POST",
            dataType: "json",
            dataSrc: "data",
    ...
    table.on('click', '.delete-btn', function () {
        const id = $(this).data('id');
        if (confirm("Bu öğe kategorisini silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.")) {
            $.ajax({
                url: `/_route/api/api/category/items/${id}`, type: 'DELETE', success: function (result) {
                    console.info(result);
