$(document).ready(function () {
    ...
    table.DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "/_route/datatables/elasticsearch/product_category",
            type: "POST",
            dataType: "json",
            dataSrc: "data",
    ...
    table.on('click', '.delete-btn', function () {
        ...
        if (confirm("Bu ürün kategorisini silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.")) {
            $.ajax({
                url: `/_route/api/api/category/products/${id}`, type: 'DELETE', success: function (result) {
                    console.info(result);
    ...
    $('#editForm').on('submit', function (e) {
        ...
        $.ajax({
            url: `/_route/api/api/category/products/${id}`,
            type: 'PATCH',
