$(document).ready(function () {
    const table = $('#menuTypeTable');

    table.DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "/_route/datatables/elasticsearch/menu_type",
            type: "POST",
            dataType: "json",
            dataSrc: "data",
    ...
    table.on('click', '.delete-btn', function () {
        ...
        if (confirm("Bu türü silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.")) {
            $.ajax({
                url: `/_route/api/api/type/menus/${id}`, type: 'DELETE', success: function (result) {
                    console.info(result);
    ...
    $('#editForm').on('submit', function (e) {
        ...
        $.ajax({
            url: `/_route/api/api/type/menus/${id}`,
            type: 'PATCH',
